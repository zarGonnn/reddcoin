...
CKeyPool CWallet::GenerateNewKey()
{
    ...
    CExtKey xsecret;
    if (!HDGenerateSecret(xsecret, nChild, false))
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %s RDD %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (fFound || wtx.GetDepthInMainChain() > 0)
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s RDD %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
bool CWallet::GetStakeWeight(const CKeyStore& keystore, uint64_t& nAverageWeight, uint64_t& nTotalWeight)
{
    // Choose coins to use
...
{
    {
        ...
        {
            CExtKey xsecret;
            if (!HDGenerateSecret(xsecret, i, false))
            ...
            {
                vUsedHDKey.push_back(pubkey.GetID());
                vKeyPool.clear();
                vSecret.clear();
            }
            else
...
{
    {
        ...
        while (setKeyPool.size() < nTargetSize)
        {
            ...
            mapKeyPool[nEnd] = GenerateNewKey();
...
void CWallet::ReserveKeyFromKeyPool(int64_t& nIndex, CKeyPool& keypool)
{
    ...
    keypool.vchPubKey = CPubKey();
    {
        ...
        if (!mapKeyPool.count(nIndex))
            throw runtime_error("ReserveKeyFromKeyPool() : read failed");
        keypool = mapKeyPool[nIndex];
...
void CWallet::GetAllReserveKeys(set<CKeyID>& setAddress)
{
    ...
    {
        CKeyPool keypool;
        if (!mapKeyPool.count(id))
            throw runtime_error("GetAllReserveKeyHashes() : read failed");
        keypool = mapKeyPool[id];
        assert(keypool.vchPubKey.IsValid());
