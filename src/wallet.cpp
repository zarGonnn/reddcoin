...
CFeeRate CWallet::minTxFee = CFeeRate(1000000);  // Override with -mintxfee
...
void CWallet::AddToSpends(const uint256& wtxid)
{
    ...
    if (thisTx.IsCoinBase() || thisTx.IsCoinStake()) // Coinbases don't spend anything!
        return;
...
// Reddcoin
void CWallet::RemoveFromSpends(const COutPoint& outpoint)
{
    pair<TxSpends::iterator, TxSpends::iterator> range;
    range = mapTxSpends.equal_range(outpoint);

    mapTxSpends.erase(range.first, range.second);
}
        ...
        if (fInsertedNew)
        {
            ...
            if (wtxIn.hashBlock != 0)
            {
                if (mapBlockIndex.count(wtxIn.hashBlock))
                {
                    ...
                    if (wtx.nTime == 0)
                    {
                        *const_cast<unsigned int*>(&(wtx.nTime)) = blocktime;
                    }
                }
                else
...
void CWallet::AvailableCoinsMinConf(vector<COutput>& vCoins, int nConf) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                isminetype mine = IsMine(pcoin->vout[i]);
                if (!(IsSpent((*it).first, i)) && mine != ISMINE_NO &&
                    !IsLockedCoin((*it).first, i) && pcoin->vout[i].nValue >= nMinimumInputValue)
